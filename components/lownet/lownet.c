...
void lownet_inbound_handler(const esp_now_recv_info_t* info,
...
void lownet_init(lownet_cipher_fn encrypt_fn, lownet_cipher_fn decrypt_fn) {
  ...
  xTaskCreatePinnedToCore(lownet_service_main, "lownet_service", 4096,
                          NULL,  // Pass no params into task method.
                          LOWNET_SERVICE_PRIO, &net_system.lownet_task,
                          LOWNET_SERVICE_CORE);

  xTaskCreate(decrypt_service_main, "decrypt_service", 2048, NULL,
              LOWNET_SERVICE_PRIO, &net_system.decrypt_task);

  // Block until the service has finished its own startup and is ready to use.
  EventBits_t startup_result = xEventGroupWaitBits(
      net_system.events, (EVENT_CORE_READY | EVENT_CORE_ERROR), pdFALSE,
      pdFALSE, TIMEOUT_STARTUP);
  ...
  ESP_LOGI(TAG, "Initialized LowNet -- device ID: 0x%02X",
           net_system.identity.node);
}
...
#error "IVT  size violation"
  ...
  if (esp_now_send(net_system.broadcast.mac, (const uint8_t*)&cipher,
                   sizeof(cipher)) != ESP_OK) {
    ESP_LOGE(TAG, "LowNet Frame send error");
  }
}

// Public interface; standard send.  Delegate to encrypt-and-send method if AES
...
void lownet_send(const lownet_frame_t* frame) {
  ...
  out_frame.source =
      net_system.identity.node;  // Overwrite frame source with device ID.
  ...
  if (lownet_get_key() != NULL) {
    ...
    if (esp_now_send(net_system.broadcast.mac, (const uint8_t*)&out_frame,
                     sizeof(out_frame)) != ESP_OK) {
      ESP_LOGE(TAG, "LowNet Frame send error");
    }
  }
}
...
void lownet_set_time(const lownet_time_t* time) {
  // Overwrite the network time with the received time.  Take the processor
...
uint8_t lownet_get_device_id() { return net_system.identity.node; }
...
const char* lownet_get_signing_key() { return net_system.signing_key; }
...
void lownet_service_main(void* pvTaskParam) {
  ...
  xEventGroupSetBits(net_system.events, EVENT_CORE_READY);
  ...
  while (1) {
    ...
    if (xQueueReceive(net_system.inbound, &frame, UINT32_MAX) == pdTRUE) {
      ...
      // Not strictly to spec but a useful safety valve; if frame has, as a
      // source address, the broadcast address, discard it -- something has gone
      // wrong.
      if (frame.source == 0xFF) {
        continue;
      }
...
void lownet_service_kill() {
  ...
  return;  // Should never execute, when this function is called from lownet
           // service.
}
...
void lownet_inbound_handler(const esp_now_recv_info_t* info,
  ...
  } else if (len == sizeof(lownet_secure_frame_t) &&
             net_system.aes_key.size != 0) {
...
uint32_t lownet_crc(const lownet_frame_t* frame) {
  ...
  void process_byte(uint8_t b) {
    for (int i = 0; i < 8; i++) {
      reg = (reg << 1) | (b & 1);
      b = b >> 1;
      if (reg & 0x1000000ul) reg = (reg ^ poly);  // take mod G(x)
    }
  }
  const uint8_t* iter = (const uint8_t*)frame;
  int len = LOWNET_FRAME_SIZE - LOWNET_CRC_SIZE;
  for (int i = 0; i < len; i++) process_byte(iter[i]);
  return reg;
}
...
int lownet_register_protocol(uint8_t protocol, lownet_recv_fn handler) {
  if (net_system.num_protocols >= LOWNET_MAX_PROTOCOLS) return 1;
