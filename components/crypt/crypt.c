#include "crypt.h"

#include <aes/esp_aes.h>
#include <esp_log.h>
#include <lownet.h>
#include <serial_io.h>
#include <stdio.h>
#include <stdlib.h>
#include <string.h>

void crypt_decrypt(const lownet_secure_frame_t* cipher,
                   lownet_secure_frame_t* plain) {
  // ...
  // TODO: Finish the decrypt function

  // Again we preserve magic, source, destination and ivt and copy the ivt
  memcpy(plain, cipher, LOWNET_UNENCRYPTED_SIZE + LOWNET_IVT_SIZE);

  uint8_t ivt[LOWNET_IVT_SIZE];
  memcpy(ivt, cipher->ivt, LOWNET_IVT_SIZE);

  // Decryption
  esp_aes_context ctx;

  esp_aes_init(&ctx);

  const lownet_key_t* key = lownet_get_key();

  esp_aes_setkey(&ctx, key->bytes, AES_KEYBITS);

  esp_aes_crypt_cbc(&ctx, ESP_AES_DECRYPT, LOWNET_ENCRYPTED_SIZE, ivt,
                    (const unsigned char*)&cipher->protocol,
                    (unsigned char*)&plain->protocol);
  esp_aes_free(&ctx);
}

void crypt_encrypt(const lownet_secure_frame_t* plain,
                   lownet_secure_frame_t* cipher) {
  // ...
  // TODO: Finish the encrypt function

  // We first need to preserve magic, source, destination and ivt in the cipher
  // frame
  memcpy(cipher, plain, LOWNET_UNENCRYPTED_SIZE + LOWNET_IVT_SIZE);

  // Then we need to create a copy of the iv since the encryption may mutate the
  // iv in place
  uint8_t ivt[LOWNET_IVT_SIZE];
  memcpy(ivt, plain->ivt, LOWNET_IVT_SIZE);

  // Now for the encryption
  esp_aes_context ctx;

  esp_aes_init(&ctx);

  const lownet_key_t* key = lownet_get_key();

  esp_aes_setkey(&ctx, key->bytes, AES_KEYBITS);

  // Everything from protocol and onwards is encrypted
  esp_aes_crypt_cbc(&ctx, ESP_AES_ENCRYPT, LOWNET_ENCRYPTED_SIZE, ivt,
                    (const unsigned char*)&plain->protocol,
                    (unsigned char*)&cipher->protocol);
  esp_aes_free(&ctx);
}

// Usage: crypt_command(KEY)
// Pre:   KEY is a valid AES key or NULL
// Post:  If key == NULL encryption has been disabled
//        Else KEY has been set as the encryption key to use for
//        lownet communication.
void crypt_setkey_command(char* args) {
  // ...
  // TODO: Finish the setkey command
  // If no key is provided encryption is disabled and the device reverts back to
  // normal P2 operation.

  if (!args) {
    lownet_set_key(NULL);
    serial_write_line("No key given: Encryption disabled");
    return;
  }

  // Construct key from the args
  // If 0 or 1 then use stored keys
  // Else use the arguments as the key

  if (!strcmp(args, "0") || !strcmp(args, "1")) {
    // key = lownet_keystore_read((uint8_t)*args);
    lownet_set_stored_key((uint8_t)(*args - '0'));
    char buf[MSG_BUFFER_LENGTH];
    sprintf(buf, "Encryption key set to stored key with key id: %s", args);
    serial_write_line(buf);
    return;
  }

  // Lownet key structure.  Bytes member MUST point to a usable contiguous
  // region of memory of AT LEAST 'size' bytes.
  lownet_key_t key;
  key.size = LOWNET_KEY_SIZE_AES;
  key.bytes = calloc(LOWNET_KEY_SIZE_AES, sizeof(uint8_t));

  size_t length = strlen(args);
  if (length > key.size) length = key.size;
  memcpy(key.bytes, args, length);

  lownet_set_key(&key);
  serial_write_line("Encryption key set to custom key");
}

void crypt_test_command(char* str) {
  if (!str) return;
  if (!lownet_get_key()) {
    serial_write_line("No encryption key set!");
    return;
  }

  // Encrypts and then decrypts a string, can be used to sanity check your
  // implementation.
  lownet_secure_frame_t plain;
  lownet_secure_frame_t cipher;
  lownet_secure_frame_t back;

  memset(&plain, 0, sizeof(lownet_secure_frame_t));
  memset(&cipher, 0, sizeof(lownet_secure_frame_t));
  memset(&back, 0, sizeof(lownet_secure_frame_t));

  const uint8_t cipher_magic[2] = {0x20, 0x4e};

  memcpy(plain.magic, cipher_magic, sizeof cipher_magic);
  plain.source = lownet_get_device_id();
  plain.destination = 0xFF;
  plain.protocol = 0x02;
  plain.length = strlen(str);

  *((uint32_t*)plain.ivt) = 123456789;
  strcpy((char*)plain.payload, str);

  crypt_encrypt(&plain, &cipher);

  if (memcmp(&plain, &cipher, LOWNET_UNENCRYPTED_SIZE) != 0) {
    serial_write_line("Unencrypted part of frame not preserved!");
    return;
  }
  if (memcmp(&plain.ivt, &cipher.ivt, LOWNET_IVT_SIZE) != 0) {
    serial_write_line("IVT not preserved!");
    return;
  }

  crypt_decrypt(&cipher, &back);

  if (memcmp(&plain, &back, sizeof plain) == 0) {
    serial_write_line("Encrypt/Decrypt successful");
    return;
  }

  serial_write_line("Encrypt/Decrypt failed");
  char msg[200];
  snprintf(msg, sizeof msg,
           "Unencrypted content: %s\n"
           "IVT:                 %s\n"
           "Encrypted content:   %s\n",
           memcmp(&plain, &back, LOWNET_UNENCRYPTED_SIZE) == 0 ? "Same"
                                                               : "Different",
           memcmp(&plain.ivt, &back.ivt, LOWNET_IVT_SIZE) == 0 ? "Same"
                                                               : "Different",
           memcmp(&plain.protocol, &back.protocol, LOWNET_ENCRYPTED_SIZE) == 0
               ? "Same"
               : "Different");
  serial_write_line(msg);
}
